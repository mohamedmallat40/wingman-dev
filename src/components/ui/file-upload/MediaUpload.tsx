...
import { Badge, Button, Card, CardBody, Chip, Input, Progress } from '@heroui/react';
...
export const MediaUpload: React.FC<MediaUploadProps> = ({
  ...
  const handleFilesSelected = useCallback(
    async (selectedFiles: File[]) => {
      ...
      if (selectedFiles.length > remainingSlots) {
        ...
        console.warn(
          `Can only add ${remainingSlots} more files. Maximum ${maxFiles} files allowed.`
        );
      }
  ...
  const isCurrentlyUploading =
    currentUploadingFile !== null || Object.keys(uploadingFiles).length > 0;
  ...
  if (!isClient) {
    return (
      <div className={`space-y-4 ${className}`}>
        <div className='border-default-300 relative rounded-xl border-2 border-dashed p-8'>
        ...
        <motion.div
          className={`relative rounded-xl border-2 border-dashed transition-all duration-300 ${
            isCurrentlyUploading
              ? 'border-default-200 bg-default-50 dark:bg-default-900/10 cursor-not-allowed opacity-50'
              : isDragOver
          ...
          <div className='flex flex-col items-center justify-center p-8 text-center'>
            ...
            <div className='mb-6 flex items-center justify-center gap-8'>
              <div className='flex flex-col items-center gap-2'>
                <Icon icon='solar:camera-outline' className='text-primary h-6 w-6' />
                <span className='text-default-600 text-xs font-medium'>Images</span>
              </div>
              <div className='flex flex-col items-center gap-2'>
                <Icon icon='solar:videocamera-outline' className='text-secondary h-6 w-6' />
                <span className='text-default-600 text-xs font-medium'>Videos</span>
              </div>
            </div>
        ...
        <div className='space-y-3'>
          ...
          {(currentUploadingFile || Object.keys(uploadingFiles).length > 0) && (
            <motion.div
              ...
              className='from-primary-50 to-secondary-50 dark:from-primary-950/20 dark:to-secondary-950/20 border-primary-200/30 dark:border-primary-800/30 relative overflow-hidden rounded-2xl border bg-gradient-to-br p-6 shadow-lg'
            >
              {/* Animated background pattern */}
              <div className='absolute inset-0 opacity-10'>
                <div className='from-primary-500/20 to-secondary-500/20 absolute inset-0 animate-pulse bg-gradient-to-r via-transparent' />
                <div className='bg-primary-400/10 animate-float absolute top-0 left-0 h-32 w-32 rounded-full blur-3xl' />
                <div
                  className='bg-secondary-400/10 animate-float absolute right-0 bottom-0 h-24 w-24 rounded-full blur-2xl'
              ...
              <div className='relative space-y-4'>
                ...
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    <div className='relative'>
                      <div className='border-primary-200 border-t-primary-500 dark:border-primary-800 dark:border-t-primary-400 h-8 w-8 animate-spin rounded-full border-3' />
                      <div className='bg-primary-500/20 absolute inset-0 animate-ping rounded-full' />
                    </div>
                    <div>
                      <h4 className='text-primary-700 dark:text-primary-300 text-sm font-semibold'>
                        Uploading Media
                      </h4>
                      <p className='text-primary-600/70 dark:text-primary-400/70 text-xs'>
                        Processing {totalFiles} file{totalFiles !== 1 ? 's' : ''} sequentially
                      </p>
                    </div>
                  </div>
                  <div className='text-right'>
                    <div className='text-primary-700 dark:text-primary-300 text-lg font-bold'>
                      {completedFiles}/{totalFiles}
                    </div>
                    <div className='text-primary-600/70 dark:text-primary-400/70 text-xs'>
                      completed
                    </div>
                  </div>
                </div>
                ...
                <div className='space-y-2'>
                  <div className='flex items-center justify-between'>
                    ...
                    <span className='text-default-600 dark:text-default-400 font-mono text-xs'>
                      {Math.round((completedFiles / totalFiles) * 100)}%
                    </span>
                  </div>
                  <div className='relative'>
                    <div className='bg-default-200 dark:bg-default-800 h-2 overflow-hidden rounded-full'>
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${(completedFiles / totalFiles) * 100}%` }}
                        transition={{ duration: 0.5, ease: 'easeOut' }}
                        className='from-primary-500 via-primary-400 to-secondary-500 relative h-full rounded-full bg-gradient-to-r'
                      >
                ...
                {currentUploadingFile && (
                  <motion.div
                    ...
                    className='space-y-3 rounded-xl border border-white/60 bg-white/50 p-4 dark:border-white/10 dark:bg-black/20'
                  >
                    <div className='flex items-center justify-between'>
                      <div className='flex items-center gap-2'>
                        <Icon
                          icon='solar:cloud-upload-bold'
                          className='text-primary-500 h-4 w-4 animate-bounce'
                        />
                        <span className='text-default-700 dark:text-default-300 text-xs font-medium'>
                          Current File
                        </span>
                    ...
                    <div className='space-y-1'>
                      <div className='text-default-600 dark:text-default-400 truncate text-xs font-medium'>
                        📁 {currentUploadingFile}
                      </div>
                      <div className='relative'>
                        <div className='bg-default-200 dark:bg-default-700 h-1.5 overflow-hidden rounded-full'>
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${Object.values(uploadingFiles)[0] || 0}%` }}
                            transition={{ duration: 0.2, ease: 'easeOut' }}
                            className='from-primary-500 to-secondary-500 relative h-full rounded-full bg-gradient-to-r'
                          >
