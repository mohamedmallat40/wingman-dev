...
export const useCommentUI = (): UseCommentUIReturn => {
  ...
  const getCommentUIState = useCallback(
    (commentId: string): CommentUIState => {
      return (
        uiState[commentId] || {
          isExpanded: false,
          isReplying: false,
  ...
  const updateCommentUIState = useCallback(
    (commentId: string, updates: Partial<CommentUIState>) => {
      setUIState((prev) => ({
        ...prev,
        [commentId]: {
  ...
  const setReplying = useCallback(
    (commentId: string, isReplying: boolean) => {
  ...
  const setEditing = useCallback(
    (commentId: string, isEditing: boolean) => {
      updateCommentUIState(commentId, {
        isEditing,
  ...
  const setReplyInput = useCallback(
  ...
  const setEditInput = useCallback(
    (commentId: string, value: string) => {
